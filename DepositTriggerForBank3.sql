...
-- =============================================
CREATE TRIGGER updateDeposit_Check 
   ON  [dbo].[checkDeposit]
   AFTER  INSERT
AS 
BEGIN
	...
	if exists (select * from inserted)
		begin
			select @depNum = CheckDepositID from inserted
			select @depAmount = CheckAmount from inserted
			select @depDate = DepositDate from inserted
